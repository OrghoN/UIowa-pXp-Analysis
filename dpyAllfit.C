...
//      "Lorentzian Peak on Quadratic Background",500,-0.5,0.5);
   hdpyAll->SetMarkerStyle(21);
   hdpyAll->SetMarkerSize(0.8);
//hdpyAll->SetStats(0);
   ...
   hdpyAll->Fit("fitFcn","0");

   // second try: set start values for some parameters
   fitFcn->SetParameter(4,0.01); // width
   fitFcn->SetParameter(5,0);   // peak
   hdpyAll->GetYaxis()->SetRangeUser(0,54000);
   hdpyAll->Fit("fitFcn","V+","ep");
...
// writes the fit results into the par array
   ...
   legend->AddEntry(hdpyAll,"Data","lpe");
...
double r = signalFcn->Integral(-0.1,0.1);
std::cout << " r = " << r << std::endl

double rr = backFcn->Integral(-0.1,0.1);
std::cout << " rr = " << rr << std::endl

double rrr = fitFcn->Integral(-0.1,0.1);
std::cout << " rrrr = " << rrr << std::endl

double rrr = fitFcn->Integral(-0.5,0.5);
