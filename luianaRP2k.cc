...
//---------------------------------------------------
  ...
  histosTH1F["hntrkq0"] = new TH1F("hntrkq0","Ntrk for nPixelHits>0 Q=0",150,0,150);
  ...
  histosTH1F["hm2rec2KK"] = new TH1F("hm2rec2KK","M_{KK} ",massbins,0,5.);
  histosTH1F["hm2recKKpid"] = new TH1F("hm2recKKpid","M_{KK} PID=kaon",massbins,0,5.);
  histosTH1F["hm2rec2KKpid"] = new TH1F("hm2rec2KKpid","M_{KK} PID=kaon",massbins,0,5.);

  histosTH2F["hm2recKK2dim"] = new TH2F("hm2recKK2dim","M_{KK} vs pt_{KK}",2*massbins,0,5.,massbins,0,5.);
  histosTH2F["hm2rec2KK2dim"] = new TH2F("hm2rec2KK2dim","M_{KK} vs pt_{KK}",2*massbins,0,5.,massbins,0,5.);
  histosTH2F["hm2recKKpid2dim"] = new TH2F("hm2recKKpid2dim","M_{KK} PID=kaon vs pt_{KK}",2*massbins,0,5.,massbins,0,5.);
  histosTH2F["hm2rec2KKpid2dim"] = new TH2F("hm2rec2KKpid2dim","M_{KK} PID=kaon vs pt_{KK}",2*massbins,0,5.,massbins,0,5.);
  ...
  //...Luiz K+K-
  histosTH2F["hdedxq0"] = new TH2F("hdedxq0","dE/dx vs p Q=0", 500, 0.,5.,1000, 0.,200.);
  histosTH2F["hdedxfidq0"] = new TH2F("hdedxfidq0","dE/dx vs p Q=0 fiducial", 500, 0.,5.,1000, 0.,200.);
  histosTH2F["hdedxKK"] = new TH2F("hdedxKK","dE/dx vs p only K+K-", 500, 0.,5.,1000, 0.,200.);
  histosTH1F["hntrkKK"] = new TH1F("hntrkKK","# of K+K-",5,0,5);
  //
  histosTH2F["hdedxq0nopix"] = new TH2F("hdedxq0nopix","dE/dx vs p Q=0 nopixelhits", 500, 0.,5.,1000, 0.,200.);
  histosTH2F["hdedxfidq0nopix"] = new TH2F("hdedxfidq0nopix","dE/dx vs p Q=0 fiducial nopixelhits", 500, 0.,5.,1000, 0.,200.);
  histosTH2F["hdedxKKnopix"] = new TH2F("hdedxKKnopix","dE/dx vs p only K+K- nopixelhits", 500, 0.,5.,1000, 0.,200.);
  histosTH1F["hntrkKKnopix"] = new TH1F("hntrkKKnopix","# of K+K- nopixelhits",5,0,5);
...
//      fout<<run<<" "<<ls<<" "<<evt<<" "<<tb<<" "<<xi_proton_left<<" "<<xi_proton_right<<" "<<ThxL<<" "<<ThxR<<" "<<ThyL<<" "<<ThyR<<" "<<xVtxL<<" "<<xVtxR<<" "<<HFveto<<endl;
	 ...
	 histosTH1F["hthxEla2_ttbb"]->Fill(ThxL+ThxR, wei);
       ...
       double pidarray[2]={0.,0.};
       double vtxdxyarray[2]={0.,0.};
       double vtxdzarray[2]={0.,0.};
	 ...
	 ntrkvtx = itVtx->ntracks;
       ...
       //...Kaons
       fiducialRegionK   = (ntrk==2 && TMath::Abs(k1.Eta())<etaCut && TMath::Abs(k2.Eta())<etaCut);  
       fiducialRegionPtK = (ntrk==2 && k1.Pt()>ptCut && k2.Pt()>ptCut);
	 ...
	 histosTH1F["hrapy2"]->Fill(rapy);	 
       ...
       //...checking dEdx efficiency K+K- 
	   //...fiducial: each entry in the scatter plot dE/dx vs p is a track!
       int ntrkKK=0;
       int ntrkKKnopix=0;
       for(vector<MyTracks>::iterator itTrack = track_coll->begin() ; itTrack != track_coll->end() ; ++itTrack){
	int npixelhitsKK = itTrack->nValidPixelHits;
	if(npixelhitsKK>0){
	 if(totcharge==0){
	   histosTH2F["hdedxq0"]->Fill(itTrack->p,itTrack->harmonic2_dEdx);}
	 //
	 if(fiducialRegionK && fiducialRegionPtK && totcharge==0){
	   histosTH2F["hdedxfidq0"]->Fill(itTrack->p,itTrack->harmonic2_dEdx);}
	 //
	 if(fiducialRegionK && fiducialRegionPtK && totcharge==0 && pidarray[0]==2 && pidarray[1]==2){
	   histosTH2F["hdedxKK"]->Fill(itTrack->p,itTrack->harmonic2_dEdx);
	   ntrkKK++;
	   }
	 } //...end of npixelhitsKK>0
	//
	//if(npixelhitsKK>0){
	 if(totcharge==0){
	   histosTH2F["hdedxq0nopix"]->Fill(itTrack->p,itTrack->harmonic2_dEdx);}
	 //
	 if(fiducialRegionK && fiducialRegionPtK && totcharge==0){
	   histosTH2F["hdedxfidq0nopix"]->Fill(itTrack->p,itTrack->harmonic2_dEdx);}
	 //
	 if(fiducialRegionK && fiducialRegionPtK && totcharge==0 && pidarray[0]==2 && pidarray[1]==2){
	   histosTH2F["hdedxKKnopix"]->Fill(itTrack->p,itTrack->harmonic2_dEdx);
	   ntrkKKnopix++;
	   }
	 //} //...end of npixelhitsKK>0
	 // std::cout << " ntrkvee = " << ntrkvee << std::endl;       
       ...
       //
	 histosTH1F["hntrkKK"]->Fill(ntrkKK);
         histosTH1F["hntrkKKnopix"]->Fill(ntrkKKnopix);
	 ...
	 if(RPvertex && CTpxcut && CTvertex){
	   ...
	   if(totcharge==0){
	     ...
	     //histosTH1F["hm2rec2KK"]->Fill(mrecKK);      
	   ...
	   if(totcharge==0 && diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec2OS_diag_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec2OS_diag_trkM"]->Fill(mrec);
	   }      
	   if(totcharge==0 && !diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec2OS_ttbb_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec2OS_ttbb_trkM"]->Fill(mrec);
	   }
	   
	   if(totcharge==0 && diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec2OS_diag_pypxP"]->Fill(mrec);
	     else histosTH1F["hm2rec2OS_diag_pypxM"]->Fill(mrec);
	     
	     int pypx=0;
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) pypx=1;
	     else pypx=0;
	     if(mrec>=1.65 && mrec<=1.75) cout<<"scan2OSdiag: "<<run<<" "<<LS<<" "<<evt<<" "<<mrec<<" "<<pypx<<endl;
	   }      
	   if(totcharge==0 && !diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec2OS_ttbb_pypxP"]->Fill(mrec);
	 ...
	 if(RPvertex && CTpxcut){
	   ...
	   if(totcharge==0 && diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec3OS_diag_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec3OS_diag_trkM"]->Fill(mrec);
	   }      
	   if(totcharge==0 && !diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec3OS_ttbb_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec3OS_ttbb_trkM"]->Fill(mrec);
	   }
	   
	   if(totcharge==0 && diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec3OS_diag_pypxP"]->Fill(mrec);
	     else histosTH1F["hm2rec3OS_diag_pypxM"]->Fill(mrec);
	   }      
	   if(totcharge==0 && !diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec3OS_ttbb_pypxP"]->Fill(mrec);
	 ...
	 if(RPvertex && CTpxcut && CTvertex && TMath::Abs(zvtx)<5.){ // core
	   ...
	   if(totcharge==0 && diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec4OS_diag_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec4OS_diag_trkM"]->Fill(mrec);
	   }      
	   if(totcharge==0 && !diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec4OS_ttbb_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec4OS_ttbb_trkM"]->Fill(mrec);
	   }
	   
	   if(totcharge==0 && diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec4OS_diag_pypxP"]->Fill(mrec);
	     else histosTH1F["hm2rec4OS_diag_pypxM"]->Fill(mrec);
	   }      
	   if(totcharge==0 && !diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec4OS_ttbb_pypxP"]->Fill(mrec);
	 ...
	 if(RPvertex && CTvertex){
	   ...
	   if(totcharge==0 && diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec5OS_diag_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec5OS_diag_trkM"]->Fill(mrec);
	   }     
	   if(totcharge==0 && !diag){
	     if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec5OS_ttbb_trkP"]->Fill(mrec);
	     if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec5OS_ttbb_trkM"]->Fill(mrec);
	   }
	   
	   if(totcharge==0 && diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec5OS_diag_pypxP"]->Fill(mrec);
	     else histosTH1F["hm2rec5OS_diag_pypxM"]->Fill(mrec);
	   }      
	   if(totcharge==0 && !diag){
	     if(TMath::Abs(pipiRec.Py()) > TMath::Abs(pipiRec.Px())) histosTH1F["hm2rec5OS_ttbb_pypxP"]->Fill(mrec);
	 ...
	 if(RPvertex && CTpxcut && CTvertex){
	   ...
	   if(TMath::Abs(pi1.Eta())<etaCut2 && TMath::Abs(pi2.Eta())<etaCut2){
	     ...
	     if(totcharge==0 && diag){
	       if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec6OS_diag_trkP"]->Fill(mrec);
	       if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec6OS_diag_trkM"]->Fill(mrec);
	     }      
	     if(totcharge==0 && !diag){
	       if(pi1.Py()*pi2.Py()>0) histosTH1F["hm2rec6OS_ttbb_trkP"]->Fill(mrec);
	       if(pi1.Py()*pi2.Py()<0) histosTH1F["hm2rec6OS_ttbb_trkM"]->Fill(mrec);
	     }
	 ...
	 if(diag && totcharge==0 &&RPvertex && CTpxcut){
       ...
       double ptKK=kkRec.Pt();
       
       if(fiducialRegionK && fiducialRegionPtK){
	 //...!CTpycut
	 if(totcharge==0){
	 histosTH1F["hm2recKK"]->Fill(mrecKK);
	 histosTH2F["hm2recKK2dim"]->Fill(ptKK,mrecKK);
	    if(pidarray[0]==2 && pidarray[1]==2){
	    histosTH1F["hm2recKKpid"]->Fill(mrecKK);      
    	    histosTH2F["hm2recKKpid2dim"]->Fill(ptKK,mrecKK);
	    }
	 }
         if(!CTpycut) continue;
     	 if(RPvertex && CTpxcut && CTvertex){   
         if(totcharge==0){
	 histosTH1F["hm2rec2KK"]->Fill(mrecKK);      
	 histosTH2F["hm2rec2KK2dim"]->Fill(ptKK,mrecKK);
	    if(pidarray[0]==2 && pidarray[1]==2){
	    histosTH1F["hm2rec2KKpid"]->Fill(mrecKK);      
    	    histosTH2F["hm2rec2KKpid2dim"]->Fill(ptKK,mrecKK);
	    }
	  }
	 }
       }
       ...
       //-----------------------------    
       // track variables
       if(ntrk==2 && CTpycut && CTpxcut && RPvertex){
	 if(totcharge==0 && diag){
	   
	   histosTH1F["hptRes"]->Fill(pipiRec.Pt());
	   histosTH1F["hetaRes"]->Fill(pipiRec.Eta());
	   histosTH1F["hphiRes"]->Fill(pipiRec.Phi());
